...
function PollOptionComponent({ option, index, options, totalVotes, withPollNode, }) {
    ...
    return (<div className="PollNode__optionContainer">
      <div className={joinClasses('PollNode__optionCheckboxWrapper', checked && 'PollNode__optionCheckboxChecked')}>
        <input ref={checkboxRef} className="PollNode__optionCheckbox" type="checkbox" onChange={(e) => {
            withPollNode((node) => {
      ...
      <div className="PollNode__optionInputWrapper">
        ...
        <input className="PollNode__optionInput" type="text" value={text} onChange={(e) => {
            withPollNode((node) => {
                node.setOptionText(option, e.target.value);
            });
        }} placeholder={`Option ${index + 1}`}/>
      </div>
      <button disabled={options.length < 3} className={joinClasses('PollNode__optionDelete', options.length < 3 && 'PollNode__optionDeleteDisabled')} arial-label="Remove" onClick={() => {
            withPollNode((node) => {
...
export default function PollComponent({ question, options, nodeKey, }) {
    ...
    return (<div className={`PollNode__container ${isFocused ? 'focused' : ''}`} ref={ref}>
      <div className="PollNode__inner">
        <h2 className="PollNode__heading">{question}</h2>
        {options.map((option, index) => {
            const key = option.uid;
            return (<PollOptionComponent key={key} withPollNode={withPollNode} option={option} index={index} options={options} totalVotes={totalVotes}/>);
        })}
