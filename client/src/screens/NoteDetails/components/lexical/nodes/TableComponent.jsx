...
import { $addUpdateTag, $createParagraphNode, $createRangeSelection, $getNodeByKey, $getRoot, $getSelection, $isNodeSelection, $isRangeSelection, CLICK_COMMAND, COMMAND_PRIORITY_LOW, COPY_COMMAND, createEditor, CUT_COMMAND, FORMAT_TEXT_COMMAND, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, KEY_ARROW_UP_COMMAND, KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND, KEY_ENTER_COMMAND, KEY_ESCAPE_COMMAND, KEY_TAB_COMMAND, PASTE_COMMAND, } from 'lexical';
import { useCallback, useContext, useEffect, useMemo, useRef, useState, } from 'react';
import * as React from 'react';
import { createPortal } from 'react-dom';
import { IS_APPLE } from '../shared/environment';
import { CellContext } from '../plugins/TablePlugin';
import { $isTableNode, cellHTMLCache, cellTextContentCache, createRow, createUID, exportTableCellsToHTML, extractRowsFromHTML, } from './TableNode';
const NO_CELLS = [];
...
function TableCellEditor({ cellEditor }) {
    ...
    return (<LexicalNestedComposer initialEditor={cellEditor} initialTheme={cellEditorConfig.theme} initialNodes={cellEditorConfig.nodes} skipCollabChecks={true}>
      {cellEditorPlugins}
...
function TableActionMenu({ cell, rows, cellCoordMap, menuElem, updateCellsByID, onClose, updateTableNode, setSortingOptions, sortingOptions, }) {
    ...
    <div className="dropdown" ref={dropDownRef} onPointerMove={(e) => {
            e.stopPropagation();
        }} onPointerDown={(e) => {
            e.stopPropagation();
        }} onPointerUp={(e) => {
      ...
      {cell.type === 'header' && y === 0 && (<>
          {sortingOptions !== null && sortingOptions.x === x && (<button className="item" onClick={() => {
          ...
          {(sortingOptions === null ||
                sortingOptions.x !== x ||
                sortingOptions.type === 'descending') && (<button className="item" onClick={() => {
          ...
          {(sortingOptions === null ||
                sortingOptions.x !== x ||
                sortingOptions.type === 'ascending') && (<button className="item" onClick={() => {
      ...
      <hr />
      {rows[0].cells.length !== 1 && (<button className="item" onClick={() => {
                updateTableNode((tableNode) => {
                    $addUpdateTag('history-push');
      ...
      {rows.length !== 1 && (<button className="item" onClick={() => {
                updateTableNode((tableNode) => {
                    $addUpdateTag('history-push');
...
function TableCell({ cell, cellCoordMap, cellEditor, isEditing, isSelected, isPrimarySelected, theme, updateCellsByID, updateTableNode, rows, setSortingOptions, sortingOptions, }) {
    ...
    return (<CellComponent className={`${theme.tableCell} ${isHeader ? theme.tableCellHeader : ''} ${isSelected ? theme.tableCellSelected : ''}`} data-id={cell.id} tabIndex={-1} style={{ width: cellWidth !== null ? cellWidth : undefined }}>
      {isPrimarySelected && (<div className={`${theme.tableCellPrimarySelected} ${isEditing ? theme.tableCellEditing : ''}`}/>)}
      {isPrimarySelected && isEditing ? (<TableCellEditor cellEditor={cellEditor}/>) : (<>
          <div style={{ position: 'relative', zIndex: 3 }} dangerouslySetInnerHTML={{
      ...
      {isPrimarySelected && !isEditing && (<div className={theme.tableCellActionButtonContainer} ref={menuRootRef}>
          <button className={theme.tableCellActionButton} onClick={(e) => {
      ...
      {showMenu &&
            menuElem !== null &&
            createPortal(<TableActionMenu cell={cell} menuElem={menuElem} updateCellsByID={updateCellsByID} onClose={() => setShowMenu(false)} updateTableNode={updateTableNode} cellCoordMap={cellCoordMap} rows={rows} setSortingOptions={setSortingOptions} sortingOptions={sortingOptions}/>, document.body)}
...
export default function TableComponent({ nodeKey, rows: rawRows, theme, }) {
    ...
    useEffect(() => {
        ...
        return mergeRegister(editor.registerCommand(CLICK_COMMAND, (payload) => {
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(PASTE_COMMAND, handlePaste, COMMAND_PRIORITY_LOW), editor.registerCommand(COPY_COMMAND, handleCopy, COMMAND_PRIORITY_LOW), editor.registerCommand(CUT_COMMAND, (event, activeEditor) => {
            if (handleCopy(event, activeEditor)) {
                clearCellsCommand();
                return true;
            }
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_BACKSPACE_COMMAND, clearCellsCommand, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_DELETE_COMMAND, clearCellsCommand, COMMAND_PRIORITY_LOW), editor.registerCommand(FORMAT_TEXT_COMMAND, (payload) => {
            if (primarySelectedCellID !== null && !isEditing) {
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ENTER_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
            if (primarySelectedCellID === null &&
                !isEditing &&
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_TAB_COMMAND, (event) => {
            const selection = $getSelection();
            if (!isEditing &&
                selection === null &&
                primarySelectedCellID !== null) {
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_UP_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_DOWN_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
            if (!isEditing && selection === null) {
                const extend = event.shiftKey;
                const cellID = extend
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_LEFT_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
            if (!isEditing && selection === null) {
                const extend = event.shiftKey;
                const cellID = extend
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_RIGHT_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
            if (!isEditing && selection === null) {
                const extend = event.shiftKey;
                const cellID = extend
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ESCAPE_COMMAND, (event, targetEditor) => {
            const selection = $getSelection();
    ...
    return (<div style={{ position: 'relative' }}>
      <table className={`${theme.table} ${isSelected ? theme.tableSelected : ''}`} ref={tableRef} tabIndex={-1}>
        <tbody>
          {rows.map((row) => (<tr key={row.id} className={theme.tableRow}>
              {row.cells.map((cell) => {
                const { id } = cell;
                return (<TableCell key={id} cell={cell} theme={theme} isSelected={selectedCellSet.has(id)} isPrimarySelected={primarySelectedCellID === id} isEditing={isEditing} sortingOptions={sortingOptions} cellEditor={cellEditor} updateCellsByID={updateCellsByID} updateTableNode={updateTableNode} cellCoordMap={cellCoordMap} rows={rows} setSortingOptions={setSortingOptions}/>);
            })}
            </tr>))}
        </tbody>
      </table>
